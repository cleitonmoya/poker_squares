...
Simula com um jogo diversas curvas
@author: cleiton
"""
...
c_max = 10000     # número máximo de passos sem melhoria
verb = False

# Agendas de resfriamento
Ti = 5
Tf = 0.001

an0 = annealing(tipo='greedy', N=N)
an1 = annealing(tipo='linear', N=N, Ti=Ti, Tf=Tf)

beta = (Tf/Ti)**(1/N)
an2 = annealing(tipo='exp', N=N, Ti=Ti, beta=beta)

a = 3.5
b = 2 
an3 = annealing(tipo='log', N=N, a=a, b=b)
Annealing = [an0, an1, an2]
...
for F in FF:
    ax[0].plot(F, linewidth=0.7)
ax[0].set_xlabel('Passos')
ax[0].set_ylabel('Score')


for Ann in Annealing:
    ax[1].plot(Ann)
ax[1].set_xlabel('Passos')
ax[1].set_ylabel('Temperatura')
ax[1].legend(['T=0', 'Linear', 'Exp', 'Log'])
plt.tight_layout()